...
## Estimación de la densidad condicional (estimadores kernel)
...
## Estimación de la matriz de mala clasificación y del error de mala clasificacion aparente (peligro de optimismo)

mod.lda <- lda(Species ~ ., data = iris2)
mod.lda

## representacion de las dos grupos de acuerdo con las dos primeras variables canónica de clasificación
...
## Representamos  de forma simultanea la frontera y los datos (utilizamos las variables canónicas de discriminacion) 
## Uso de la función partimat de la libreria klaR
...
## Uso de la libreria ggplot y representacion simultanea de datos y contornos de densidad (estimación kernel)
...
## Uso de la fucion stepclass de klaR. improvement=0.001 o más pequeño para que entren 2 o más variables
...
## funcion para resumir la capacidad clasificatoria de un método
...
## Primer intento de obtener una estimación con menor optimismo
## uso de CV=TRUE
...
###################################
# Tarea: Repetir el análisis con los datos crabs de la libreria(MASS)
####################################
...
## añadir suavizadores para detectar efecto no lineal

termplot(pima.glm1, partial.resid=TRUE, se=TRUE, rug=TRUE, smooth=panel.smooth, span.smth=1/5)

## Suele ser necesario un número grande de casos para
...
# utilizamos una función de la libreria boot

library(boot)
# esta función cuenta la proporción de errores de predicción

cost <- function(r, pi=0) mean(abs(r-pi)>0.5)

# error de predicción con cross-validation K-fold
...
table(pima0$type,fitpima.glm2 > 0.5)
